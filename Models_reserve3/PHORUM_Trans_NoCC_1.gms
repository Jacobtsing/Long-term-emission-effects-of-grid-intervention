...
HOURLY_COSTc(t) ..   HourlyCost(t) =e= sum(g, gLevel(g, t)*(gVC(g)) + gStartupCost(g, t) + gNLC(g)*U(g,t)+gReserve(g,t)*gReserveCost(g))+sum(s,2*sReserve(s,t));
STARTUPCOSTc(g,t) .. gStartupCost(g,t) =g= gStartupC(g)*(U(g,t) - U(g,t-1));

* Load constraints for each TCR
SUPPLYTCR1c(t)$(ord(t) gt 1) ..            loadTCR1(t) =e= sum(gTCR1, gLevel(gTCR1, t)) - TI12(t) - TI13(t) - TI15(t) + windTCRact1(t) + solarTCRact1(t) + sum(sTCR1,sDischarge(sTCR1,t)-sCharge(sTCR1,t));
SUPPLYTCR2c(t)$(ord(t) gt 1) ..            loadTCR2(t) =e= sum(gTCR2, gLevel(gTCR2, t)) + TI12(t) + TI52(t) - TI23(t) + windTCRact2(t) + solarTCRact2(t) + sum(sTCR2,sDischarge(sTCR2,t)-sCharge(sTCR2,t));
SUPPLYTCR3c(t)$(ord(t) gt 1) ..            loadTCR3(t) =e= sum(gTCR3, gLevel(gTCR3, t)) + TI23(t) + TI13(t) - TI34(t) + windTCRact3(t) + solarTCRact3(t) + sum(sTCR3,sDischarge(sTCR3,t)-sCharge(sTCR3,t));
SUPPLYTCR4c(t)$(ord(t) gt 1) ..            loadTCR4(t) =e= sum(gTCR4, gLevel(gTCR4, t)) + TI34(t)  + windTCRact4(t) + solarTCRact4(t) + sum(sTCR4,sDischarge(sTCR4,t)-sCharge(sTCR4,t));
SUPPLYTCR5c(t)$(ord(t) gt 1) ..            loadTCR5(t) =e= sum(gTCR5, gLevel(gTCR5, t)) + TI15(t) - TI52(t) + windTCRact5(t) + solarTCRact5(t) + sum(sTCR5,sDischarge(sTCR5,t)-sCharge(sTCR5,t));
...
RESERVETCR2c(t)$(ord(t) gt 1) ..            0.01* loadTCR2(t) + 0.005 * windTCRact2(t) + 0.003 * solarTCRact2(t) =l= 
                                            sum(gTCR2, gReserve(gTCR2,t))
                                            +sum(sTCR2,sReserve(sTCR2,t));
RESERVETCR3c(t)$(ord(t) gt 1) ..            0.01* loadTCR3(t) + 0.005 * windTCRact3(t) + 0.003 * solarTCRact3(t) =l= 
                                            sum(gTCR3, gReserve(gTCR3,t))
                                            +sum(sTCR3,sReserve(sTCR3,t));
RESERVETCR4c(t)$(ord(t) gt 1) ..            0.01* loadTCR4(t) + 0.005 * windTCRact4(t) + 0.003 * solarTCRact4(t) =l= 
...
RESERVETCR2spinc(t)$(ord(t) gt 1) ..            0.03* loadTCR2(t) =l= 
                                            sum(gTCR2, gReservespin(gTCR2,t))
                                            +sum(sTCR2,sReservespin(sTCR2,t));
RESERVETCR3spinc(t)$(ord(t) gt 1) ..            0.03* loadTCR3(t) =l= 
                                            sum(gTCR3, gReservespin(gTCR3,t))
                                            +sum(sTCR3,sReservespin(sTCR3,t));
RESERVETCR4spinc(t)$(ord(t) gt 1) ..            0.03* loadTCR4(t) =l= 
...
* CC0, Trans1
execute_unload "results.gdx" modelStatus solveStatus gap gLevel U HourlyCost gVC sDischarge sReserve gReserve sReservespin gReservespin sCharge sSOC loadTCR1 loadTCR2 loadTCR3 loadTCR4 loadTCR5 windTCRact1 windTCRact2 windTCRact3 windTCRact4 windTCRact5 solarTCRact1 solarTCRact2 solarTCRact3 solarTCRact4 solarTCRact5 SUPPLYTCR1c.m SUPPLYTCR2c.m SUPPLYTCR3c.m SUPPLYTCR4c.m SUPPLYTCR5c.m TI12 TI15 TI13 TI52 TI23 TI34 TI12max TI13max TI15max TI52max TI23max TI34max RESERVETCR1c.m RESERVETCR2c.m RESERVETCR3c.m RESERVETCR4c.m RESERVETCR5c.m RESERVETCR1spinc.m RESERVETCR2spinc.m RESERVETCR3spinc.m RESERVETCR4spinc.m RESERVETCR5spinc.m 
* CC1, Trans0
